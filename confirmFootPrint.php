                        ...
                        <div class="container">
                            ...
                            if (is_file("./csv/footPrint.csv")) { //ファイルが存在するか
                                if (is_readable("./csv/footPrint.csv")) { //ファイルを読み込めるか
                                    $fp1 = fopen("./csv/footPrint.csv", "r"); //足跡情報
                                    flock($fp1, LOCK_SH);
                                    while (!feof($fp1)) {
                                        $content = fgetcsv($fp1);
                                        //自分のidの検索
                                        if ($content[1] == $_SESSION['id']) {
                                            ...
                                            if (is_file("./csv/profile.csv")) { //ファイルが存在するか
                                                if (is_readable("./csv/profile.csv")) { //ファイルを読み込めるか
                                                    $fp2 = fopen("./csv/profile.csv", "r"); //足跡情報
                                                    flock($fp2, LOCK_SH);
                                                    while (!feof($fp2)) {
                                                        $profile = fgetcsv($fp2);
                                                        //送った相手のid検索
                                                        if ($content[0] == $profile[0] && $profile[0] != $_SESSION['id']) {
                                                            //URLパラメータ生成
                                                            print '<a href="userProfile.php?id=' . $profile[0] . '"><div class="item"><img src = ' . $profile[1] . ' align="left" width="128" height="128" class="userImage" alt=""><p>　' . $profile[3] . ' </p><p>　' . $profile[4] . ' </p><hr  style="border:1px dashed #000000;"><p>　' . $profile[2] . ' </p></div></a>';
                                                            break;
                                                        }
                                                    }
                                                    flock($fp2, LOCK_UN);
